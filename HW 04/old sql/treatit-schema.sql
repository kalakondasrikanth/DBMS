...
CREATE TRIGGER AddPatientTrigger
	...
	FOR EACH ROW
	EXECUTE PROCEDURE AlterPatient(1);
...
CREATE FUNCTION AlterPatient(i int) RETURNS TRIGGER AS $$
DECLARE
BEGIN

	--i>0 IF I HAVE INSERTED A NEW ROW IN Takes_care (i.e. THE DOCTOR OF THE CORRESPONDING ROW
	--HAS A NEW PATIENT, SO I NEED TO INCREMENT HIS ACTUAL PATIENTS BY ONE UNIT
	--ELSE: I CALLED A DELETION, SO THE NUMBER OF PATIENTS OF THE CORRESPONDING DOCTOR DECREASED BY ONE UNIT
	IF i>0 THEN
